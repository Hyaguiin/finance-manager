...
class TransactionAnalysisService {
    createTransactionAnalysis = async (analysisData) => {
        try {
    ...
    getTransactionAnalysisById = async (id) => {
        try {
            if (!id) {
                throw new ErrorMissingContent();
            }
    ...
    updateTransactionAnalysis = async (id, updateData) => {
        try {
            const analysis = await TransactionAnalysisModel.findOne({ where: { id } });
            if (!analysis) {
                throw new NotFound();
    ...
    deleteTransactionAnalysis = async (id) => {
        try {
            const analysis = await TransactionAnalysisModel.findOne({ where: { id } });
            if (!analysis) {
                throw new NotFound();
